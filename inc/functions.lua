...
message = 'ʕ•◡-ʔ**قائمه المحظورين عام :\n'
for k,v in pairs(list) do
local info = redis:hgetall(bita..'username:'..v)
if info and info.username and info.username:match("@[%a%d_]+") then
message = message ..k.. '~⪼ '..(info.username or '')..' « ❪`' ..v.. '`❫ \n'
...
if redis:get(bita..'group:add'..msg.chat_id_) then  return 'ʕ•◡-ʔ**المجموعه بالتأكيد تم تفعيلها' end
...
return sendMsg(msg.chat_id_,msg.id_,'- **عذرا البوت ليس ادمن  في المجموعه ʕ•◡-ʔ\n-**يرجى ترقيته ادمن لتتمكن من تفعيل البوت  ')
else
local lock_service = redis:get(bita..'lock_service')
if lock_service then 
sendMsg(msg.chat_id_,msg.id_,'ʕ•◡-ʔ*تم تفعيل المجموعه . \n*ʕ•◡-ʔ ورفع كل المشرفين\n [𝑪𝑯𝑨𝑵𝑬𝑳 𝑺𝑶𝑼𝑹𝑪𝑬 𓏢](t.me/baselabaza)')
else
sendMsg(msg.chat_id_,msg.id_,'ʕ•◡-ʔتـم تـفـعـيـل آلمـجمـوعه‏‏ \n ')
end
GetChannelAdministrators(msg.chat_id_,function(arg,data)
for k,v in pairs(data.members_) do
...
USERNAME_T = 'ʕ•◡-ʔ**الـمعرف ❪ @['..datai.username_..'] ❫\n'
else 
USERNAME_T = ''
end
...
if not redis:get(bita..'group:add'..msg.chat_id_) then return '- ** المجموعه بالتأكيد تم تعطيلها' end  
rem_data_group(msg.chat_id_)
return 'ʕ•◡-ʔ**The group has been disabled .'
end

function modrem(msg)
if not msg.SudoUser then return 'ʕ•◡-ʔ**أنـت لـسـت الـمـطـور'end
if not redis:get(bita..'group:add'..msg.chat_id_) then return 'ʕ•◡-ʔ**المجموعه بالتأكيد تم تعطيلها' end  
rem_data_group(msg.chat_id_)
return 'تم تعطيل المجموعه ، لتفعيلها مره اخرى ارسل تفعيلʕ•◡-ʔ'
end
...
return SendMention(ChatID,UserID,MsgID,'ʕ•◡-ʔ 𝑼𝑺𝑬 » '..namei..' \n ʕ•◡-ʔ 𝑴𝑺𝑮 » ❪ '..maseegs..' ❫ \nʕ•◡-ʔ 𝑨𝑪𝑻𝑰𝑽𝑬 »  ❪ '..Get_Ttl(maseegs)..' ❫\n',12,utf8.len(namei)) 
end
...
if cmd == "whois" then
return sendMsg(ChatID,MsgID,
  'ʕ•◡-ʔ 𝑵𝑨𝑴𝑬 » '..FlterName(data.title_,30)..'\n'
..'ʕ•◡-ʔ 𝑰𝑫 » {`'..UserID..'`} \n'
..' ʕ•◡-ʔ 𝑼𝑺𝑬 »  '..UserName..'\n'
..'ʕ•◡-ʔ 𝑺𝑻𝑨 » '..Getrtba(UserID,ChatID)..'\n'
..'\n'
...
.."\n11-│عدد التكرار «« "..(redis:get(bita..'num_msg_max'..msg.chat_id_) or 'false')
